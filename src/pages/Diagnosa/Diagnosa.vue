<template>
  <b-container fluid>
      ...
      <div class="col-12">
        ...
        >
          ...
          >
            ...
            <template v-if="role == 'admin'" #cell(actions)="row">
              <button
    ...
    >
      <pre>
are you sure want to delete <strong>{{ infoModal.title }} </strong>from diagnosa list ?</pre>
    </b-modal>
  </b-container>
</template>
...
import Diagnosa from "@/api/DiagnosaApi";
...
export default {
  ...
  methods: {
    ...
    async handleOk(id) {
      ...
      try {
        let res = await Diagnosa.Delete(id);
    ...
    info(item, index, button) {
      console.log(item);
      this.infoModal.title = `hapus diagnosa ini?`;
    ...
    async loadStart() {
      try {
        ...
        if (data_user.role == "user") {
          res = await Diagnosa.GetByUser(data_user._id);
        } else {
          res = await Diagnosa.Get();
        }
