...
export default {
  ...
  methods: {
    ...
    async onSubmit(evt) {
      ...
      try {
        let res = await Rule.Update(this.$route.params.id, fix_data);
        if (res.data.success) {
          this.success = true;
          this.$notify({
  ...
  async created() {
    this.isLoading = true;
    let res_rule = await Rule.Detail(this.$route.params.id);
    if (res_rule.data.success) {
      this.form.id_penyakit = res_rule.data.data[0].id_penyakit._id;
      this.form.gejala = res_rule.data.data[0].gejala.map((el) => {
